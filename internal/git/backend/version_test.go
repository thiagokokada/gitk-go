...
func TestParseGitVersionOutput(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "plain",
			in:   "git version 2.44.0\n",
			want: gitVersion{major: 2, minor: 44, patch: 0},
			ok:   true,
		},
		{
			name: "apple_git",
			in:   "git version 2.39.3 (Apple Git-146)\n",
			want: gitVersion{major: 2, minor: 39, patch: 3},
			ok:   true,
		},
		{
			name: "windows_suffix",
			in:   "git version 2.39.3.windows.1\n",
			want: gitVersion{major: 2, minor: 39, patch: 3},
			ok:   true,
		},
		{
			name: "no_prefix",
			in:   "2.42.1\n",
			want: gitVersion{major: 2, minor: 42, patch: 1},
			ok:   true,
		},
		{
			name: "no_patch",
			in:   "git version 2.42\n",
			want: gitVersion{major: 2, minor: 42, patch: 0},
			ok:   true,
		},
		{
			name: "invalid",
			in:   "git version not-a-version\n",
			ok:   false,
		},
	}
